...
兩項都由**新聞管道**解決（PRD §4.2／§8.2 明訂未達門檻即觸發此備案）。
...
| **減資參考價** | TWSE `TWTAUU` | 394 事件（**PRD 未涵蓋，見 §5.1**） |
...
依 PRD §5.3 的判讀表，「週末 → Turnover 顯著、週末 → ROI 不顯著」落在
...
1.30 倍，5% 門檻因此翻轉。本專案採較保守的做法（PRD §5.1 C9，理由是 267 檔
...
**本專案的結論是：證據不足以區分兩條管道。** 這正是 PRD §2.3 明確允許的結果——
...
這些不是 PRD 預期到的問題，而是實作與驗證過程中發現的。每一項都有回歸測試鎖住。
...
（Yahoo 還原股本變動但不還原股利），因此構成 PRD §3.6 要求的第三方驗證：
...
| 4 | 推文有作者與時戳 | **無**。1,103 萬則推文無法定位，`low_effort` 依 PRD 備案改為「不含推文」 |
| 5 | 分類清單八類 | 實測缺「情報」（9.4%）等；PRD 列的「問卷」僅佔 0.02% |
...
1. **新聞管道**（鉅亨網，方案已驗證）——一次解掉兩個未通過的驗收門檻：
   `news_count` 是 PRD §4.2 明訂的主表必要控制項，缺它 H1 的資訊解讀有未排除的
   替代假說；且新聞覆蓋率遠高於 PTT，可補足 `dense` 檔數。§5.9 的需求端 vs. 供給端
   比較**在 H1 邊緣時特別有價值**。
2. **人工複核 LLM 判讀**——本專案的抽驗由 LLM 執行，PRD §8.1 要求的是人工。
