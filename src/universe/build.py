...
GENERIC_DEMOTION: dict[str, str] = {
    ...
    # 抽驗實證後新增（2026-09，audit/adjudication/）：簡稱指涉集團旗下**另一家**
    # 公司的比例過高，鄰接阻擋規則無法處理「等大型券商」「三商巧福」這類列舉語境。
    "1216": "統一（統一證券／投信／投顧／獅／動詞「統一」：20 筆抽驗 18 筆誤配）",
    "2905": "三商（三商壽／銀／餐飲／家購／巧福／人壽：17 筆抽驗全數誤配）",
    "1806": "冠軍（銷售冠軍／冠軍教練／交投冠軍：抽驗 6 筆全數為普通名詞）",
    # 消費品牌污染
...
# 阻擋延伸清單：命中片段被相鄰字擴成**另一家公司**時不採計。
# 全部來自 2026-09 的分層抽驗實證（audit/adjudication/），不是憑空臆測：
#   統一 20 筆中 18 筆誤配、三商 17 筆全數誤配。
BLOCKED_AFTER: dict[str, list[str]] = {
    "1216": ["證", "投信", "投顧", "投", "超", "獅", "發票", "數位",
             "夢公園", "時代", "生機", "速達", "陽光"],   # 統一企業本身保留
    "2905": ["壽", "銀", "美邦", "餐飲", "家購", "電", "行"],
    "1304": ["集"],                                        # 平台聚集
    "2605": ["市場", "國家", "經濟體", "產業", "債", "電商",
             "科技", "應用", "經濟"],                       # 新興市場／新興債
    "1907": ["生技"],                                      # 永豐餘生技（未上市）
    "1605": ["科"],                                        # 華新科（非本檔）
    "1603": ["子"],                                        # 聯華電子／福華電子
...
BLOCKED_BEFORE: dict[str, list[str]] = {
    "1304": ["平"],       # 平台聚集／平台聚寶匯
    "1603": ["中", "聯", "福", "大"],   # 中華電／聯華電子／福華電子
    "2605": ["-", "－"],                # 券商分點「第一金-新興」
    "2002": ["台灣", "興"],
    "1216": ["台灣"],
}
    ...
    for row in rows:
        ...
        for t in texts:
            if t and t not in seen:
                seen.add(t)
                entry = {"text": t, "match_mode": mode, "priority": len(t)}
                if ticker in BLOCKED_AFTER:
                    entry["blocked_after"] = BLOCKED_AFTER[ticker]
                if ticker in BLOCKED_BEFORE:
                    entry["blocked_before"] = BLOCKED_BEFORE[ticker]
